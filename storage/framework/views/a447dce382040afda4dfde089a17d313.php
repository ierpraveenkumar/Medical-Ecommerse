...
<?php $__env->startSection('content'); ?>
    <div class="container mx-auto my-10 p-6 bg-white rounded-md shadow-md">
        ...
        <!-- Date filter -->
        <form action="<?php echo e(route('admin.manage.shipped.orders')); ?>" method="get" class="mb-4">
            <?php echo csrf_field(); ?>
            <div class="flex mb-4 items-center">
                ...
                <input type="date" class="border rounded-md p-2" name="end_date" value="<?php echo e(request('end_date')); ?>">
                        ...
                        <?php if($order->invoice): ?>
                            <tr>
                                <td class="py-2 px-4"><input type="checkbox" name="selectedOrders[]"
                                        value="<?php echo e($order->id); ?>"></td>
                                <td class="py-2 px-4"><?php echo e($order->id); ?></td>
                                <td class="py-2 px-4"><?php echo e($order->shipping_address); ?></td>
                                <td class="py-2 px-4"><?php echo e($order->invoice->id); ?></td>
                                <td class="py-2 px-4"><?php echo e($order->invoice->created_at->format('d-m-Y')); ?></td>
                                <td class="py-2 px-4">
                                    <?php echo e(app('App\Http\Controllers\AdminController')->generateTrackingId($order->invoice->id)); ?>

                                </td>
                                <td class="py-2 px-4"><a href="<?php echo e(route('manage-shipped-view', $order->id)); ?>"
    ...
    $(document).ready(function() {
        $('#invoicedownload').click(function(e) {
            ...
            if ($('input[name="selectedOrders[]"]:checked').length === 0) {
                ...
                $.ajax({
                    url: '<?php echo e(route('orders.invoices.shipped')); ?>',
                    type: 'POST',
                    data: formData,
    ...
    $(document).ready(function() {
        $('#exportshippedselected').click(function(e) {
            ...
            //if rows selected
                ...
                $.ajax({
                    url: '<?php echo e(route('export.selected.shipped')); ?>',
                    type: 'POST',
                    data: formData,
                    success: function(response) {
                        console.log(response);
...
<?php echo $__env->make('Layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/CompletedProjectsSPX066/admin_panel/resources/views/Admin/Pages/manage-shipped-orders.blade.php ENDPATH**/ ?>